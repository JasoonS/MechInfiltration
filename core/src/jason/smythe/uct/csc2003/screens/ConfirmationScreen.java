...
public class ConfirmationScreen implements Screen {
	...
	public ConfirmationScreen(GameSoundController sound, PlayController playController) {
		super();
		this.sound = sound;
	...
	public void show() {
		...
		heading = new Label("Are you happy with your selection?\nMaster Mechanics never rush their thought process\nAre you sure you want to start infiltration?", new LabelStyle(font, Color.WHITE));
//		heading.setFontScale(2);
		
		exitButton = new TextButton("No, I think I can do better.", textButtonStyle);
		exitButton.addListener(new ClickListener() {

			@Override
			public void clicked(InputEvent event, float x, float y) {
				((Game) Gdx.app.getApplicationListener()).setScreen(new GearBeltSetup(sound, playController));
			}
		});
		exitButton.pad(15);
		
		playButton = new TextButton("Yes, lets play!", textButtonStyle);
